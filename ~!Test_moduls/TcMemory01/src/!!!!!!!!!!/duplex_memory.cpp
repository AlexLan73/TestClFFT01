...
//  _memoryRead = std::make_unique<MemoryBase>(readChannelName, TypeBlockMemory::Read, callBack);
//
//  // Создаем экземпляр MemoryBase для записи. Колбэк ему не нужен.
//  _memoryWrite = std::make_unique<MemoryBase>(writeChannelName, TypeBlockMemory::Write, nullptr);
//}
//
