...
socket_test::socket_test(int id, unsigned short listen_port, unsigned short connect_port): wrap_thread(id)
{
    sockets_ = std::make_shared<DuplexNode>(io_, listen_port, connect_port);
}
...
void socket_test::run()
{
    ...
    //        static_cast<unsigned short>(10003));
    //    node->start();
...
/*        auto node = std::make_shared<DuplexNode>(io_context,
//            static_cast<unsigned short>(std::stoi(argv[1])),
//            static_cast<unsigned short>(std::stoi(argv[2])));
*/
        sockets_->start();
